...
# number of radars and cameras
num_radars = 5
num_cameras = 6

# radar fov boundary points
# radar_fov_ranges = np.array([0,0,40,70,70,70,150,210,250,293.3747,250,210,150,70,70,70,40,0,0])
# radar_fov_azimuths = np.array([-90,-60.1,-60,-40,-20,-9,-9,-6,-4,0,4,6,9,9,20,40,60,60.1,90])

radar_fov_ranges = np.array([0,0,40,293.3747,40,0,0])
radar_fov_azimuths = np.array([-90,-60.1,-60,0,60,60.1,90])
...
# Radar measurement attributes ( csv file )
"""
...
radar_id = {radar_location_attr[i]:i for i in range(len(radar_location_attr))}
radar_id_to_name = {i:radar_location_attr[i] for i in range(len(radar_location_attr))}

camera_location_attr = ['front', 'front_left', 'rear_left', 'rear', 'rear_right', 'front_right']
camera_id = {camera_location_attr[i]:i for i in range(len(camera_location_attr))}
camera_id_to_name = {i:camera_location_attr[i] for i in range(len(camera_location_attr))}
