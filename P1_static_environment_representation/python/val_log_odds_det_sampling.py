...
from lib_log_odds import (
    kernel,
    compute_meas_log_likelihood_deterministic_sampling,
    inflate_prob_and_compute_log_odds,
    compute_prob_from_log_odds
)


def plot_meas(meas_x, meas_y, prob):
    _, ax = plt.subplots(1,1)
    ax.scatter(meas_y, meas_x, alpha=prob, s=0.1, color='red', marker='.', label='cell coordinates')
    ax.set_aspect('equal')
...
if __name__ == '__main__':
    ...
    sample_grid = kernel(num_rows=20, num_cols=20)
    ...
    prob, cellids, xcoord, ycoord = compute_meas_log_likelihood_deterministic_sampling(
        meas_stationary_x,
        meas_stationary_y, 
        0.5,
        sample_grid,
