...
# description : Library for Grid Cell State Update ( kalman filtering )
# =====================================================================================================================
...
def cell_states_prediction(
    ...
    var_xcoord = np.clip(var_xcoord[condition] + config.process_noise, a_min=None, a_max=config.max_noise)
    var_ycoord = np.clip(var_ycoord[condition] + config.process_noise, a_min=None, a_max=config.max_noise)
    cellids = grid_properties.compute_scalar_ids_from_xy_coordinates(xcoord, ycoord)

    return ( logodds, cellids, xcoord, ycoord, var_xcoord, var_ycoord )
...
def update_cell_coordinates(
    ...
    # -------------------------------------------------------------------------------------
    var_upd = np.zeros((num_cells, 2), dtype=np.float32)
    var_pred = np.zeros((num_cells, 2), dtype=np.float32)
    var_meas = np.zeros((num_cells, 2), dtype=np.float32)
    # -------------------------------------------------------------------------------------
    var_pred[cellids_pred] = np.stack([var_xcoord_pred, var_ycoord_pred], axis=-1)
    var_meas[cellids_meas] = np.stack([var_xcoord_meas, var_ycoord_meas], axis=-1)
    # -------------------------------------------------------------------------------------
    weight_meas = var_pred[gated_cells] / ( var_pred[gated_cells] + var_meas[gated_cells] )
    weight_pred = 1 - weight_meas
    # -------------------------------------------------------------------------------------
    cell_coord_upd[ungated_cells] = cell_coord_pred_vals[ungated_cells]
    cell_coord_upd[new_cells] = cell_coord_meas_vals[new_cells]
    cell_coord_upd[gated_cells] = weight_meas * cell_coord_meas_vals[gated_cells] + weight_pred * cell_coord_pred_vals[gated_cells]
    # -------------------------------------------------------------------------------------
    var_upd[ungated_cells] = var_pred[ungated_cells]
    var_upd[new_cells] = var_meas[new_cells]
    var_upd[gated_cells] = weight_pred * var_pred[gated_cells]
...
def sort_logodds_and_compute_cellids(
    ...
    cell_upd_states = np.stack([
        logodds_upd, 
        xcoord_upd, ycoord_upd, 
        var_xcoord_upd, var_ycoord_upd], axis=-1)

    cell_upd_states = cell_upd_states[flag]
    idx = np.argsort(cell_upd_states[:, 0])
    cell_upd_states = cell_upd_states[idx]
...
def cell_states_update(
    ...
    xcoord_upd, ycoord_upd, \
    var_xcoord_upd, var_ycoord_upd \
        = update_cell_coordinates(
            num_cells, cellids_pred, cellids_meas,
            xcoord_pred, xcoord_meas,
            ycoord_pred, ycoord_meas,
            var_xcoord_pred, var_xcoord_meas,
            var_ycoord_pred, var_ycoord_meas,
...
def log_odds_filter_step(
    ...
    xcoord_meas, ycoord_meas \
        = compute_meas_log_likelihood(
            ...
            grid_properties)
    # -------------------------------------------------------------------------------------
    # compute meas variances
    var_xcoord_meas = np.repeat(config.sampling_variance, xcoord_meas.shape[0])
    var_ycoord_meas = np.repeat(config.sampling_variance, ycoord_meas.shape[0])
    # -------------------------------------------------------------------------------------
    ...
    if trigger[radar_id]:
        ...
        var_xcoord_upd = var_xcoord_meas
        var_ycoord_upd = var_ycoord_meas
        trigger[radar_id] = False
    # -------------------------------------------------------------------------------------
    else:
        logodds_pred, cellids_pred, \
        xcoord_pred, ycoord_pred, \
        var_xcoord_pred, var_ycoord_pred \
            = cell_states_prediction(
                ...
                cell_states.get_var_px_sensor_i(radar_id), 
                cell_states.get_var_py_sensor_i(radar_id),
                T_curr, T_prev)
        ...
        var_xcoord_upd, var_ycoord_upd \
            = cell_states_update(
                ...
                xcoord_meas, ycoord_meas,
                var_xcoord_meas, var_ycoord_meas)
    # -------------------------------------------------------------------------------------
    ...
    cell_states.set_var_px_sensor_i(radar_id, var_xcoord_upd[L:])
    cell_states.set_var_py_sensor_i(radar_id, var_ycoord_upd[L:])
...
def ego_compensate_cells_other_sensors(
    ...
    for i in range(lib_const.num_radars):
        if i != radar_id and trigger[i] == False :
            ...
            cell_states.set_var_px_sensor_i(i, var_xcoord)
            cell_states.set_var_py_sensor_i(i, var_ycoord)
